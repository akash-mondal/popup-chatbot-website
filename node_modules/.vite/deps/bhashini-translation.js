...
var resolveConfig_default = (config) => {
  ...
  let { data, withXSRFToken, xsrfHeaderName, xsrfCookieName, headers: headers2, auth: auth2 } = newConfig;
...
// node_modules/bhashini-translation/config/config.js
var getModelurl = "https://meity-auth.ulcacontrib.org/ulca/apis/v0/model/getModelsPipeline";
var serviceUrl = "https://dhruva-api.bhashini.gov.in/services/inference/pipeline";

// node_modules/bhashini-translation/config/verification.js
var headers = {};
...
var verification_default = auth;

// node_modules/bhashini-translation/config/tasktypeConfig.js
var getTasktypeconfig = (srcLang, targetLang) => {
  return [
    {
      "taskType": "asr",
      "config": {
...
var tasktypeConfig_default = getTasktypeconfig;

// node_modules/bhashini-translation/config/pipelineConfig.js
var pipelineConfig = async (taskType, srcLang, targetLang) => {
  try {
    let pipelineTasks_body = "";
    if (taskType == "asr") {
      pipelineTasks_body = tasktypeConfig_default(srcLang)[0];
    } else if (taskType == "translation") {
      pipelineTasks_body = tasktypeConfig_default(srcLang, targetLang)[1];
    } else if (taskType == "tts") {
      pipelineTasks_body = tasktypeConfig_default(srcLang)[2];
    }
    const payload = {
      "pipelineTasks": [
        pipelineTasks_body
    ...
    return await axios_default.post(getModelurl, payload, { headers }).then((res) => {
...
var pipelineConfig_default = pipelineConfig;

// node_modules/bhashini-translation/config/payload.js
var input_text = (sourceText) => {
  return {
    "input": [
      {
        "source": sourceText
...
var getTtsPipeline = async (sourceLang, gender) => {
  let ttsPipeline = await pipelineConfig_default("tts", sourceLang);
...
var asr_payload = async (sourceLang, base64) => {
  const asrPipeline = await pipelineConfig_default("asr", sourceLang);
  return {
    "pipelineTasks": [asrPipeline],
    "inputData": input_base64(base64)
  };
};
var asr_nmt_payload = async (sourceLang, targetLang, base64) => {
  return {
    "pipelineTasks": [
      await pipelineConfig_default("asr", sourceLang),
      await pipelineConfig_default("translation", sourceLang, targetLang)
    ],
    "inputData": input_base64(base64)
  };
};
var asr_nmt_tts_payload = async (sourceLang, targetLang, base64, gender) => {
  const asrPipeline = await pipelineConfig_default("asr", sourceLang);
  const translationPipeline = await pipelineConfig_default("translation", sourceLang, targetLang);
...
var nmt_payload = async (sourceLang, targetLang, sourceText) => {
  const translationPipeline = await pipelineConfig_default("translation", sourceLang, targetLang);
  return {
...
var nmt_tts_payload = async (sourceLang, targetLang, sourceText, gender) => {
  const translationPipeline = await pipelineConfig_default("translation", sourceLang, targetLang);
  const ttsPipeline = await getTtsPipeline(sourceLang, gender);
  return {
...
var tts_payload = async (sourceLang, sourceText, gender) => {
  const ttsPipeline = await getTtsPipeline(sourceLang, gender);
  return {
...
// node_modules/bhashini-translation/src/utils/baseApi.js
var BaseApi = class {
  /**
  ...
  async post(data) {
    try {
      return await axios_default.post(serviceUrl, data, { headers });
    } catch (err) {
      console.log(err);
      return err;
    }
...
// node_modules/bhashini-translation/src/nmt.js
var api = new baseApi_default();
...
// node_modules/bhashini-translation/src/asr.js
var api2 = new baseApi_default();
async function asr(sourceLang, base64) {
  const payload = await asr_payload(sourceLang, base64);
  try {
    const res = await api2.post(payload);
...
// node_modules/bhashini-translation/src/utils/audioUri.js
...
var audioUri_default = getaudioUri;

// node_modules/bhashini-translation/src/tts.js
var api3 = new baseApi_default();
async function tts(sourceLang, sourceText, gender) {
  const payload = await tts_payload(sourceLang, sourceText, gender);
  try {
    const response = await api3.post(payload);
    let base64Sound = response.data.pipelineResponse[0].audio[0].audioContent;
    return audioUri_default(base64Sound);
  } catch (error) {
    console.log("error in response", error);
  }
}
...
// node_modules/bhashini-translation/src/nmt+tts.js
var api4 = new baseApi_default();
async function nmt_tts(sourceLang, targetLang, sourceText, gender) {
  ...
  try {
    const res = await api4.post(payload);
    const response = [
    ...
    let audioUri = await audioUri_default(base64Sound);
    return { translatedText, audioUri };
  } catch (error) {
    console.log("error in response", error);
  }
}
var nmt_tts_default = nmt_tts;

// node_modules/bhashini-translation/src/asr+nmt.js
var api5 = new baseApi_default();
async function asr_nmt(sourceLang, targetLang, base64) {
  const payload = await asr_nmt_payload(sourceLang, targetLang, base64);
  try {
    const res = await api5.post(payload);
    const response = [
      res.data.pipelineResponse[0].output[0].source,
...
var asr_nmt_default = asr_nmt;

// node_modules/bhashini-translation/src/asr+nmt+tts.js
var api6 = new baseApi_default();
async function asr_nmt_tts(sourceLang, targetLang, base64, gender) {
  ...
  try {
    const res = await api6.post(payload);
    const response = [
      res.data.pipelineResponse[0].output[0].source,
      // sourceText  [0]
      res.data.pipelineResponse[1].output[0].target,
      // translated text  [1]
      res.data.pipelineResponse[2].audio[0].audioContent
      // translated audio  [2]
    ];
    const base64Sound = response[2];
    return audioUri_default(base64Sound);
...
var asr_nmt_tts_default = asr_nmt_tts;

// node_modules/bhashini-translation/index.js
var bhashini = {
  asr: asr_default,
  nmt: nmt_default,
  tts: tts_default,
  nmt_tts: nmt_tts_default,
  asr_nmt: asr_nmt_default,
  asr_nmt_tts: asr_nmt_tts_default,
  auth: verification_default
};
var bhashini_translation_default = bhashini;
export {
  bhashini_translation_default as default
};
//# sourceMappingURL=bhashini-translation.js.map
